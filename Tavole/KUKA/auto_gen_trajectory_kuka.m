...
%% Load Robot's DH table and define its Joint limits 

[DH,Convention] = Load_KUKA_DH();

% joint limits (deg): (q1..q5)
%       [-166.0031 -101.0010 -166.0031 -176.0012 -166.0031  -1.0027 -166.0031] [deg]
q_min = [-2.8973   -1.7628   -2.8973   -1.5   -2.8973]'; % [rad]
q_min_ = q_min + ones(5,1)*deg2rad(5);  % give a safety margin of 5 deg
%       [166.0031  101.0010  166.0031 -3.9992   166.0031   215.0024 166.0031] [deg]
q_max =[ 2.8973    1.7628    2.8973  -0.0698    2.8973 ]'; % [rad]
q_max_ = q_max - ones(5,1)*deg2rad(5);  % give a safety margin of 5 deg
% joint velocity limits (rad/s): (q1..q5)
dq_max = [2.1750 2.1750 2.1750 2.1750 2.6100 ]';
dq_max_ = dq_max - ones(5,1)*deg2rad(10);
dq_min = -dq_max;
dq_min_ = -dq_max_;
% joint acceleration limits (rad/s^2): (q1..q5)
ddq_max = [15 7.5 10 12.5 15]';
ddq_min = -ddq_max; 
...
%         if all(abs(p) < 0.4*ones(3,1)) || all(p(1) > -0.15 & abs(p(2))< 0.5 & p(3) < 0.3 )
%             collide = true;
%             break;            
%         end
%     end
...
%  Verify positions, velocities and accelerations are within the limits
    if not(collide)
        ...
        if strcmp('y',str)
            ...
            dq_vrep = reshape(dq(:,:,tr), 5*size(dq(:,:,tr),2),1);
