%script to check ergonomy functional values over time and mean once 
%specified trial and subjects (an healthy and a stroke).
...
ntask	= 7;
h_subj	= 1;
s_subj	= 22;		% first five subjects are healthy ones
...
q_trial_h	= q_task_warp(ntask).subject(h_subj).trial(ntrial).q_grad(:,[10:end]);
...
%% figure 2 stroke subject, affected side

ntrial	= 2;
q_trial_s	= q_task_warp(ntask).subject(s_subj).trial(ntrial).q_grad(:,[10:end]);
...
clf
hold on
if q_task_warp(ntask).subject(s_subj).trial(ntrial).stroke_task == 1
...
%% figure 3 stroke subject, less affected side
