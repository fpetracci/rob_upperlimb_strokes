function data = q_trial2q_rel(trial)
...
if trial.task_side == 0 % left side
	...
	pos_shoulder_meas_rel	= pos_shoulder_meas - pos_L5_meas;
	pos_elbow_meas_rel		= pos_elbow_meas	- pos_L5_meas;
	pos_wrist_meas_rel		= pos_wrist_meas	- pos_L5_meas;
	pos_L5_meas_rel			= pos_L5_meas		- pos_L5_meas;
	...
	pos_wrist_meas_x_rel	= pos_wrist_meas_x		- pos_L5_meas;
	pos_wrist_meas_y_rel	= pos_wrist_meas_y		- pos_L5_meas;
	pos_elbow_meas_x_rel	= pos_elbow_meas_x		- pos_L5_meas;
	pos_elbow_meas_y_rel	= pos_elbow_meas_y		- pos_L5_meas;
	pos_shoulder_meas_x_rel = pos_shoulder_meas_x	- pos_L5_meas;
	pos_shoulder_meas_y_rel = pos_shoulder_meas_y	- pos_L5_meas;
	pos_L5_meas_x_rel		= pos_L5_meas_x			- pos_L5_meas;
	pos_L5_meas_y_rel		= pos_L5_meas_y			- pos_L5_meas;
...
yMeas_rel =[pos_L5_meas_x_rel		+ par.L5_pos;	...
			pos_L5_meas_y_rel		+ par.L5_pos;	...
			pos_shoulder_meas_rel	+ par.L5_pos;	...
			pos_shoulder_meas_x_rel + par.L5_pos;	...
			pos_shoulder_meas_y_rel + par.L5_pos;	...
			pos_elbow_meas_rel		+ par.L5_pos;	...
			pos_elbow_meas_x_rel	+ par.L5_pos;	...
			pos_elbow_meas_y_rel	+ par.L5_pos;	...
			pos_wrist_meas_rel		+ par.L5_pos;	...
			pos_wrist_meas_x_rel	+ par.L5_pos;	...
			pos_wrist_meas_y_rel	+ par.L5_pos];			
...
yMeas_EE_pos_rel= pos_wrist_meas_rel(:,1,1)+par.L5_pos;
TgEE_i_rel		= rt2tr(yMeas_EE_rot, yMeas_EE_pos_rel);
q0_ikunc		= arm.ikunc(TgEE_i_rel);
...
filter_horiz  = extendedKalmanFilter(...
		...
		@MeasurementNoiseFcn_rel,...% Measurement function
...
for t = 1:t_tot
	...
	[xCorrected_horiz(:,:,t), PCorrected_horiz(:,:,t)] = correct(filter_horiz, yMeas_rel(:, :, t), arm);
	...
	while k <= k_max
		...
		[xCorrected_vert(:,:,t,k), PCorrected_vert(:,:,t,k)] = correct(filter_vert, yMeas_rel(:, :, t), arm);
		predict(filter_vert);
		
		e(:,:,t,k) = yMeas_rel(:,:,t) - MeasurementFcn_rel(filter_vert.State, arm );
...
yMeas_virt_rel	= zeros(size(yMeas,1),t_tot);

for i = 1:t_tot
	yMeas_virt_rel(:,i) = fkine_kalman_marker_rel(q_rad(:,i),arm);
end

y_real_rel = reshape(yMeas_rel,size(yMeas_rel,1),size(yMeas_rel,3),size(yMeas_rel,2));
error_rel = y_real_rel(:,1:size(yMeas_virt_rel,2)) - yMeas_virt_rel;
